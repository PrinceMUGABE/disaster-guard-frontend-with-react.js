...
const UserPredictions = () => {
  ...
  // Update the fetch predictions function to include prevention strategies
  const fetchPredictions = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8000/prediction/user/", {
        headers: {
      ...
      if (response.ok) {
        ...
        // Create a map of prediction IDs to their prevention strategies
        const strategiesMap = {};
        (data.prevention_strategies || []).forEach((strategy) => {
          if (strategy.prediction && strategy.prediction.id) {
            if (!strategiesMap[strategy.prediction.id]) {
              strategiesMap[strategy.prediction.id] = [];
            }
            strategiesMap[strategy.prediction.id].push(strategy);
          }
        });
        setPredictionStrategies(strategiesMap);
      } else {
  ...
  const filteredPredictions = React.useMemo(() => {
    ...
    return predictions.filter((pred) => {
      ...
      return (
        pred?.district?.toLowerCase().includes(searchLower) ||
        pred?.sector?.toLowerCase().includes(searchLower) ||
        pred?.predictions?.most_likely_disaster
          ?.toLowerCase()
  ...
  const renderCharts = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* Add the area chart after pagination controls */}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis />
                <Tooltip
                  formatter={(value) => [`${value} predictions`, "Total"]}
  ...
  // Add this function to prepare data for the area chart
  const prepareTimeSeriesData = () => {
    ...
    // Create a map to count predictions per date
    const predictionsByDate = sortedPredictions.reduce((acc, prediction) => {
      const date = new Date(prediction.created_at).toISOString().split("T")[0];
      acc[date] = (acc[date] || 0) + 1;
      return acc;
    }, {});

    // Convert to array format for recharts with cumulative counts
    let cumulativeCount = 0;
    return Object.entries(predictionsByDate).map(([date, count]) => {
  ...
  return (
    <div className="w-full px-12 ml-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-2 md:space-y-0">
        ...
        <div className="relative w-full md:w-auto">
          ...
          <input
            ...
            className="w-full text-gray-700 md:w-auto pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="lg:w-2/3">
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {/* Add this wrapper div around the table */}
      ...
      <Dialog open={showViewModal} onOpenChange={setShowViewModal}>
        <DialogContent className="sm:max-5/6 max-h-[100vh] overflow-y-auto  py-96">
          <DialogHeader>
            <DialogTitle className="mt-16 text-sky-900">Prediction Details</DialogTitle>
          </DialogHeader>
          {selectedPrediction && (
            <div className="mt-96 space-y-6">
              {/* Prediction Details Section - Keeping this unchanged */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">
                  Prediction Information
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  {/* ... existing prediction information grid ... */}
                </div>
              ...
              {/* Reorganized Prevention Strategies Section */}
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">
                ...
                {predictionStrategies[selectedPrediction.id]?.length > 0 ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-white border rounded-lg p-4">
                        ...
                        {predictionStrategies[selectedPrediction.id][0]
                          ?.human_resources ? (
                      ...
                      <div className="bg-white border rounded-lg p-4">
                        ...
                        {predictionStrategies[selectedPrediction.id][0]
                          ...
                          <p className="text-sm text-gray-500">
                            No equipment resources specified
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <p className="text-sm text-gray-500">
                    No prevention strategies available for this prediction.
                  </p>
                )}
              </div>
